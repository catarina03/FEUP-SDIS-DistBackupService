...
public class DeleteTask extends Task {
    ...
    /**
     * Deletes all files and chunks that need to be deleted and sends the DELETED
     * message
     */
    public void run() {
        try {
            ...
            if (this.tries == 0) {
                if (!this.peer.storage.deletedFilesLocation.containsKey(this.message.header.fileId)) { // to keep record
                                                                                                       // of deleted
                                                                                                       // files
                ...
                if (this.peer.storage.files.containsKey(message.header.fileId)) { // if file is being deleted from file
                                                                                  // system, it must first exist there
