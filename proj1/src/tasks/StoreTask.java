...
public class StoreTask extends Task {
     ...
     * @param chunk  Chunk of the message received, to be stored
     */
    ...
    /**
     * Stores the chunk and sends a STORED message - simple version
     */
    private void backupAndAcknowledge() {
        ...
        ConcurrentSkipListSet<Integer> currentChunkStorageList = this.peer.storage.chunksLocation
                .computeIfAbsent(this.chunk.id, value -> new ConcurrentSkipListSet<>());
        ...
        Header storedHeader = new Header(this.peer.version, "STORED", this.peer.id, this.header.fileId,
                this.header.chunkNo);
        StoredMessage storedMessage = new StoredMessage(storedHeader, this.peer.multicastControlAddress,
    ...
    /**
     * Stores the chunk and sends a STORED message - enhanced version
     */
    private void backupAndAcknowledgeEnhanced() {
        if (!this.peer.storage.chunksLocation.containsKey(chunk.id)
            ...
            // SAVES CHUNK TO FILE DIRECTORY
            this.peer.fileManager.saveChunkToDirectory(this.chunk, this.peer.id, this.header.chunkNo,
                    this.header.fileId);
    ...
    /**
     * Checks the protocol version and chooses the backup and aknowledge function
     * accordingly
     */
    public void run() {
    ...
    /**
     * Sends the STORED message to the multicast channel
     */
    private void sendStorageMessage(byte[] messageInBytes) {
        ...
        try {
            ...
            // sending request
            DatagramPacket replyPacket = new DatagramPacket(messageInBytes, messageInBytes.length,
